...
// 기관 이름 -> 시도. **행정구역 전체 이름으로만 맞춘다.**
// '서울'·'부산' 두 글자로 맞추면 서울대학교·부산대학교가 하는 전국 사업까지
// 지역 것으로 잘못 묶인다.
const INST_SIDO = {
...
function regionOf(p) {
  ...
  // 여러 시도가 적혀 있으면 코드로는 알 수 없다. 진짜 전국 정책(국토부 등)도
  // 16개 시도를 다 적고, 의성군처럼 **지역 정책인데 다 적어 놓은 곳**도 있다.
  // 기관 칸은 정직하다 — 특히 rgtrHghrkInstCdNm 은 주관기관이 '의성군' 처럼
  // 적혀 있어도 '경상북도' 라고 제대로 들어 있다.
